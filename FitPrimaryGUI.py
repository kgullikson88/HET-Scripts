...
class LineFitter:
    ...
    def Plot(self):
        ...
        for i, order in enumerate(self.orders[start:]):
            ...
            median = np.mean(order.y)
            std = np.std(order.y)
            length = 30
            badindices = np.where(np.abs(order.y[:length] - median) / std > 5)[0]
            ...
            self.PlotArrays(
                ((order.x, order.y), (self.current_model.x, (self.current_model.y) * self.current_order.cont)),
                self.mainaxis, legend=False)
            plt.show()
