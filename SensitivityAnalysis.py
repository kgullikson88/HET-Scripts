...
if __name__ == "__main__":
    ...
    for fname in fileList:
        if extensions:
            orders_original = FitsUtils.MakeXYpoints(fname, extensions=extensions, x="wavelength", y="flux",
                                                     errors="error")
            if tellurics:
        ...
        for j, model in enumerate(model_data):
            ...
            for vel in vel_list:
                ...
                for i, order in enumerate(orders):
                    ...
                    model2.cont = FittingUtilities.Continuum(model2.x, model2.y, fitorder=3, lowreject=1.5,
                                                             highreject=10.0)
                ...
                corr = Correlate.PyCorr2(orders, resolution=60000, models=[model_data[j], ], model_fcns=None,
                                         stars=[star_list[j], ], temps=[temp_list[j], ], gravities=[gravity_list[j], ],
                                         metallicities=[metal_list[j], ], vsini=0.0, debug=False, save_output=False)[0]
